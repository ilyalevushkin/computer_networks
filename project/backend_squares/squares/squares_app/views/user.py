...
class UsersView(APIView):
    ...
    def get(self, request):
        ...
        return Response({'Users': json_data})
    ...
    def post(self, request):
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            if self.logic.users_signup_check(username):
                ...
                return Response(f'User {username} exists', status=200)
    ...
    def post(self, request):
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            if self.logic.users_signin_check(validated_data):
                token = self.logic.create_token(validated_data)
                if not token:
                    return Response('User has already logged in', status=status.HTTP_200_OK)
            else:
                return Response('User not exists or wrong password', status=status.HTTP_400_BAD_REQUEST)

        return Response(f"You were logged in with token {token}", status=status.HTTP_200_OK)
...
class UserView(APIView):
    ...
    def get(self, request, pk):
        ...
        return Response({'User': json_data})
    ...
    def patch(self, request, pk):
        if self.logic.user_is_admin(request.user) or self.logic.pk_is_users(request.user, pk):
            ...
            if serializer.is_valid(raise_exception=True):
                ...
                return Response({"Success": json_data}, status=status.HTTP_200_OK)
        else:
