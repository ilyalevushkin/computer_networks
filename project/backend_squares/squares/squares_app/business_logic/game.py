...
class GamesLogic:
    ...
    def is_user_valid(self, player_id, user_id, user_player_id):
        ...
        if player_id != -1:
            ...
            print(users.pk, user_id)
            if users.id == user_player_id:
                # есть ли я у друга в друзьях (подписан ли он на меня)
                # готов ли он играть
                print('here')
                if self.friends_logic.friend_exists(users.id, player_id) and \
                        self.friends_logic.friend_ready(player_id, users.id):
                    print('here')
                    # отменяем готовность друга
                    self.friends_manager.update_note(users.id, player_id, False)
                    return True
                elif self.pull_players_logic.user_in_pull(player_id):
                    # убираем из пула друга, чтобы начать игру
                    self.pull_players_manager.delete_note(player_id)
                    return True
        return False
